...
# ======================================================
# IMÁGENES DE TEXTO PARA VENTANAS FLOTANTES
# ======================================================
# Los textos principales de st.dialog se renderizan como PNG con letra blanca
# para evitar que Streamlit los pinte en azul oscuro sobre fondo oscuro.

def _dialog_font(size: int, bold: bool = False):
    candidates = [
...
def _wrap_text_for_image(draw, text, font_obj, max_width):
    ...
    for word in words:
        ...
        try:
            w = draw.textlength(candidate, font=font_obj)
        except Exception:
            w = len(candidate) * 8
        if w <= max_width:
            current = candidate
...
def _make_text_png(width, height, draw_callback, bg="#0F172A"):
...
def make_sicom_dialog_text_png() -> bytes:
    width, height = 900, 245
    title_font = _dialog_font(38, True)
    body_font = _dialog_font(25, False)
    label_font = _dialog_font(24, True)

    def draw_content(draw):
        y = 10
        draw.text((0, y), "Identificación de la EDS", font=title_font, fill="#FFFFFF")
        y += 62
        paragraph = (
            "Ingrese el código SICOM de la estación de servicio para consultar su mercado relevante "
            "y continuar con la evaluación del contrato."
        )
        for line in _wrap_text_for_image(draw, paragraph, body_font, width - 10):
            draw.text((0, y), line, font=body_font, fill="#F8FAFC")
            y += 34
        y += 18
        draw.text((0, y), "Código SICOM", font=label_font, fill="#FFFFFF")

    return _make_text_png(width, height, draw_content)


@st.cache_data(show_spinner=False)
def make_lectura_rapida_text_png(bucket: str) -> bytes:
    width, height = 980, 790
    title_font = _dialog_font(40, True)
    heading_font = _dialog_font(27, True)
    body_font = _dialog_font(25, False)
    bold_font = _dialog_font(25, True)

    if bucket == "Bajo":
        lectura = (
            "El contrato presenta una baja concentración de factores contractuales sensibles, según la parametrización "
            "de la herramienta. Se recomienda conservar el reporte como soporte y realizar seguimiento si se modifican "
            "las condiciones contractuales."
        )
    elif bucket == "Medio":
        lectura = (
            "El contrato presenta elementos que justifican una revisión preventiva. Se recomienda analizar con mayor "
            "detalle las condiciones contractuales antes de renovar, modificar o suscribir nuevos compromisos."
        )
    else:
        lectura = (
            "El contrato presenta una combinación de condiciones que amerita una revisión técnica detallada. Se recomienda "
            "evaluar el alcance de las cláusulas, su justificación económica y sus posibles efectos sobre la autonomía "
            "competitiva del minorista."
        )

    def draw_content(draw):
        y = 10
        x = 0
        draw.text((x, y), "Lectura rápida de resultados", font=title_font, fill="#FFFFFF")
        y += 78
        intro = (
            "Este resultado sintetiza los principales elementos contractuales que, de acuerdo con la parametrización "
            "actual del modelo, incrementan el riesgo potencial de afectación a la competencia."
        )
        for line in _wrap_text_for_image(draw, intro, body_font, width - 10):
            draw.text((x, y), line, font=body_font, fill="#F8FAFC")
            y += 35
        y += 20
        draw.text((x, y), "Lectura operativa del resultado:", font=heading_font, fill="#FFFFFF")
        y += 55
        for line in _wrap_text_for_image(draw, lectura, body_font, width - 10):
            draw.text((x, y), line, font=body_font, fill="#F8FAFC")
            y += 35
        y += 18
        interp_head = "Interpretación operativa:"
        draw.text((x, y), interp_head, font=bold_font, fill="#FFFFFF")
        head_w = draw.textlength(interp_head + " ", font=bold_font)
        interp = (
        ...
        line = ""
        x_line = x + int(head_w)
        max_w = width - x_line - 10
        for word in words:
            candidate = (line + " " + word).strip()
            if draw.textlength(candidate, font=body_font) <= max_w:
                line = candidate
            else:
                draw.text((x_line, y), line, font=body_font, fill="#F8FAFC")
                y += 35
                x_line = x
                max_w = width - 10
                line = word
        ...
        y += 30
        note_x, note_y = 0, y
        note_w, note_h = width, 150
        try:
            draw.rounded_rectangle((note_x, note_y, note_x + note_w, note_y + note_h), radius=22, fill="#1E497D")
        except Exception:
            draw.rectangle((note_x, note_y, note_x + note_w, note_y + note_h), fill="#1E497D")
        note = (
            "Nota: esta herramienta tiene un carácter preventivo y orientador. El semáforo no constituye una determinación "
            "de infracción ni sustituye el análisis jurídico o económico de fondo."
        )
        ny = note_y + 30
        for line in _wrap_text_for_image(draw, note, body_font, note_w - 70):
            draw.text((note_x + 35, ny), line, font=body_font, fill="#FFFFFF")
            ny += 35
...
#    st.title("Herramienta de identificación de riesgos de pérdida de competencia")
    ...
    try:
        with open(GUIA_USO_PATH, "rb") as pdf_file:
            guia_pdf = pdf_file.read()
...
# Refuerzo visual final V5: modales azul oscuro y texto blanco.
st.markdown("""
<style>
div[data-testid="stDownloadButton"] button,
div[data-testid="stDownloadButton"] button *,
...
""")
    ...
    if hasattr(st, "dialog"):
        ...
        def sicom_dialog():
            st.markdown(
                """
                <div style="color:#FFFFFF !important; -webkit-text-fill-color:#FFFFFF !important; margin-bottom:18px;">
                    <h2 style="color:#FFFFFF !important; -webkit-text-fill-color:#FFFFFF !important; font-size:30px; line-height:1.2; margin:0 0 22px 0; font-weight:800;">
                        Identificación de la EDS
                    </h2>
                    <p style="color:#FFFFFF !important; -webkit-text-fill-color:#FFFFFF !important; font-size:17px; line-height:1.55; margin:0 0 18px 0; font-weight:500;">
                        Ingrese el <strong style="color:#FFFFFF !important; -webkit-text-fill-color:#FFFFFF !important; font-weight:800;">código SICOM</strong> de la estación de servicio para consultar su mercado relevante y continuar con la evaluación del contrato.
                    </p>
                    <p style="color:#FFFFFF !important; -webkit-text-fill-color:#FFFFFF !important; font-size:15px; line-height:1.35; margin:0 0 8px 0; font-weight:800;">
                        Código SICOM
                    </p>
                </div>
                """,
...
#            )
            ...
            if hasattr(st, "dialog"):
                ...
                def lectura_rapida_dialog():
                    ...
                    elif res.get("bucket", "Bajo") == "Medio":
                        lectura_operativa = (
                            "El contrato presenta elementos que justifican una revisión preventiva. "
                            "Se recomienda analizar con mayor detalle las condiciones contractuales antes de renovar, "
                            "modificar o suscribir nuevos compromisos."
                    ...
                    st.markdown(
                        f"""
                        <div style="color:#FFFFFF !important; -webkit-text-fill-color:#FFFFFF !important;">
                            <h2 style="color:#FFFFFF !important; -webkit-text-fill-color:#FFFFFF !important; font-size:30px; line-height:1.2; margin:0 0 24px 0; font-weight:800;">
                                Lectura rápida de resultados
                            </h2>

                            <p style="color:#FFFFFF !important; -webkit-text-fill-color:#FFFFFF !important; font-size:17px; line-height:1.6; margin:0 0 22px 0; font-weight:500;">
                                Este resultado sintetiza los <strong style="color:#FFFFFF !important; -webkit-text-fill-color:#FFFFFF !important; font-weight:800;">principales elementos contractuales</strong> que, de acuerdo con la parametrización actual del modelo, <strong style="color:#FFFFFF !important; -webkit-text-fill-color:#FFFFFF !important; font-weight:800;">incrementan el riesgo potencial de afectación a la competencia</strong>.
                            </p>

                            <p style="color:#FFFFFF !important; -webkit-text-fill-color:#FFFFFF !important; font-size:18px; line-height:1.45; margin:0 0 18px 0; font-weight:800;">
                                Lectura operativa del resultado:
                            </p>

                            <p style="color:#FFFFFF !important; -webkit-text-fill-color:#FFFFFF !important; font-size:17px; line-height:1.6; margin:0 0 24px 0; font-weight:500;">
                                {lectura_operativa}
                            </p>

                            <p style="color:#FFFFFF !important; -webkit-text-fill-color:#FFFFFF !important; font-size:17px; line-height:1.6; margin:0 0 24px 0; font-weight:500;">
                                <strong style="color:#FFFFFF !important; -webkit-text-fill-color:#FFFFFF !important; font-weight:800;">Interpretación operativa:</strong> Un mayor número de factores activados o una mayor contribución acumulada sugiere la conveniencia de realizar una <strong style="color:#FFFFFF !important; -webkit-text-fill-color:#FFFFFF !important; font-weight:800;">revisión técnica más detallada del contrato</strong>, considerando su contexto económico y regulatorio.
                            </p>

                            <div style="background:#1E497D !important; border-radius:14px; padding:20px 24px; margin-top:18px;">
                                <p style="color:#FFFFFF !important; -webkit-text-fill-color:#FFFFFF !important; font-size:16px; line-height:1.6; margin:0; font-weight:500;">
                                    <strong style="color:#FFFFFF !important; -webkit-text-fill-color:#FFFFFF !important; font-weight:800;">Nota:</strong> esta herramienta tiene un carácter preventivo y orientador. El semáforo no constituye una determinación de infracción ni sustituye el análisis jurídico o económico de fondo.
                                </p>
                            </div>
                        </div>
                        """,
...
# V6: último refuerzo CSS para diálogos. Mantiene caja azul oscuro y fuerza texto blanco.
st.markdown("""
<style>
div[data-baseweb="modal"] div[role="dialog"],
div[role="dialog"] {
